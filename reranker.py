...
def load_step3_candidates(path):
    ...
    with path.open("r", encoding="utf-8") as step3_file:
        step3_output = json.load(step3_file)
...
#as a model we will choose cross-encoder/ms-marco-MiniLM-L-6-v2, which is fine-tuned for relevance ranking tasks and should perform well in finding subtle differences
...
#reranker function that takes the original query and the list of candidates, computes relevance scores using the cross-encoder model 
# and returns the top k reranked candidates based on those scores.
